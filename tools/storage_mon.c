...
#if 1
      	syslog(LOG_INFO, "#### YAMAUCHI #### test_device_main() start --- finished_count : %ld device_count : %ld --- final_score : %d", 
			finished_count, device_count, final_score);
#endif
	if (daemonize) {
...
syslog(LOG_INFO, "#### YAMAUCHI #### test_device_main() : response_final_score - %d", response_final_score); 
	...
	if (!daemonize) {
		...
		if (data != NULL) {
#if 1
    syslog(LOG_INFO, "#### YAMAUCHI #### test_device_main() : Next Timer start");
#endif
